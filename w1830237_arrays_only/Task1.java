...
public class Task1 {
    ...
    private static void viewAllVaccinationBooths() 
    {
        ...
        for (int count = 0; count < 6; count++) 
        {
            ...
            {
                System.out.println("Booth " + count + " is occupied by " + serviceCenter[count]);
            }
        }
    }
    ...
    // Number of vaccines remaining is also checked and if it is less than or
    // equal to 20, a warning is displayed.
    // If all booths are full and patient wasn't added, a warning is output.
    ...
    private static int addPatientToBooth(int vaccines) 
    {
        ...
        for (int count = 0; count < 6; count++) 
        {
            if (!serviceCenter[count].equals("e"))
                continue;
            serviceCenter[count] = customerName;
            patientAdded = true;
    ...
    // Show all patients currently in booths, get name of patient to remove.
    ...
    private static void removePatientFromBooth() 
    {
        ...
        if (empty == false) 
        {
            ...
            for (int count = 0; count < 6; count++) 
            {
                if (serviceCenter[count].equals(customerName)) 
                {
                    serviceCenter[count] = "e";
                    patientRemoved = true;
                    viewAllVaccinationBooths();
    ...
    {
        String[] tempArray = serviceCenter.clone();
        String temp;
