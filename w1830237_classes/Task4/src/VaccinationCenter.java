...
public class VaccinationCenter 
{
    ...
    public static void main(String[] args) throws IOException 
    {
        ...
        while (!Choice.equals("999") && !Choice.equals("EXT")) 
        {
            ...
            switch (Choice) 
            {
                ...
                case "103", "RPB" -> removePatientFromBooth(vaccineCount);
    ...
    private static int addPatientToBooth(int vaccines) 
    {
        ...
        for (int count = 0; count < 6; count++) 
        {
                    ...
                    && serviceCenter[count].getVaccineType().equals(vaccineRequest)) 
            {
                ...
                serviceCenter[count].setPatientDetails(fname, sname, age, city, nicOrPassportNumber, vaccineRequest);
        ...
        if (!patientAdded) 
        {
            System.out.println("Booths that offer that vaccine are full. Please wait for your turn.");
            Patient patient = new Patient(fname, sname, age, city, nicOrPassportNumber, vaccineRequest);
    ...
    // @no parameters
    // @no return
    private static void removePatientFromBooth(int vaccines) 
    {
        String fname = "";
        String sname = "";
        int age = 0;
        ...
        if (empty == false) 
        {
            ...
            for (int count = 0; count < 6; count++) 
            {
                if (serviceCenter[count].getPatientName().equals(customerfName)) 
                {
                    ...
                    patientRemoved = true;
                    System.out.println(customerfName + " has been removed.");
                    for (int count1 = 0; count1 < waitingList.size(); count1++) 
                    {
                        if (waitingList.get(count1).getVaccineRequested().equals(serviceCenter[count].vaccination)) 
                        {
                            fname = waitingList.get(count1).getFirstName();
                            sname = waitingList.get(count1).getSurName();
                            age = waitingList.get(count1).getAge();
                            city = waitingList.get(count1).getCity();
                            nicOrPassportNumber = waitingList.get(count1).getNICOrPassportNumber();
                            vaccineRequest = waitingList.get(count1).getVaccineRequested();

                            serviceCenter[count].setPatientDetails(fname, sname, age, city, nicOrPassportNumber,
                                    vaccineRequest);
                            waitingList.remove(count1);
                            System.out.println(fname + " " + sname + " has been added.");
                            vaccines = vaccines - 1;
                            if (vaccines <= 20) 
                            {
                                System.out.println("Warning: Vaccines in stock running out. Re-order soon.");
